...
class RelativeHuberLoss(nn.HuberLoss):
    def __init__(self, delta=1.0, eps=1e-6, reduction='mean'):
        super().__init__(delta=delta, reduction=reduction)
        self.eps = eps

    def forward(self, input, target):
        scale = target.norm(dim=-1, keepdim=True).clamp(min=self.eps)
        return super().forward(input/scale, target/scale)
